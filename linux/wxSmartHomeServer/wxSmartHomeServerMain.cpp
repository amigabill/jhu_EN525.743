...
wxSmartHomeServerFrame::wxSmartHomeServerFrame(wxWindow* parent,wxWindowID id)
{
    ...
    shPanel1 = new wxPanel(this, ID_PANEL1, wxDefaultPosition, wxSize(800,480), wxNO_BORDER|wxTAB_TRAVERSAL, _T("ID_PANEL1"));
    shPanel1->SetBackgroundColour(wxSystemSettings::GetColour(wxSYS_COLOUR_INFOBK));
    shTextCtrlEvtlog = new wxTextCtrl(shPanel1, ID_TEXTCTRL_SH_EVT_LOG, _("Text"), wxPoint(272,64), wxSize(520,368), wxTE_AUTO_SCROLL|wxTE_MULTILINE|wxTE_READONLY|wxTE_LEFT, wxDefaultValidator, _T("ID_TEXTCTRL_SH_EVT_LOG"));
...
#if 0 // these two checkboxes were intended as early dev controls/indicators for smarthome at-home status, toward automatic vacation-mode.
      // That wasnt' yet implemented at all, but keep these nearby for future use
...
// Change to the selected room number
// selcet the default load (load number 0) in this room as current load to control
// display new room.load names to GUI display
void wxSmartHomeServerFrame::selectRoom(uint16_t roomNum)
{
   uint16_t tmpNumLoadsInRoom = 0;
    ...
    {
        // invalid room number, do nothing
    }
...
void wxSmartHomeServerFrame::selectLoad(uint16_t loadNum)
{
    if( (loadNum >= 0) && (loadNum <= lastLoadNumInRoom) )
    {
        ...
        shCurrentLoadNodeInfo.SHthisNodeMsg.SHmsgType = SH_MSG_TYPE_CMD_REQ;  //SH_MSG_ST_IDLE;
        shCurrentLoadNodeInfo.SHthisNodeMsg.SHcommand = SH_CMD_LOAD_READCRNT; //SH_CMD_NOP;
        shCurrentLoadNodeInfo.SHthisNodeMsg.SHstatusH = 0;
        shCurrentLoadNodeInfo.SHthisNodeMsg.SHstatusL = 0;
        shCurrentLoadNodeInfo.SHthisNodeMsg.SHstatusID = 0;
        shCurrentLoadNodeInfo.SHthisNodeMsg.SHstatusVal = 0;
...
//        shCurrentLoadNodeInfo.SHthisNodeLevelCurrent = SHloadRequestCurrentIntensity(shCurrentLoadNodeInfo.SHthisNodeID);
        ...
        // maybe deprecated now that we draw based on command-completed message from the load
...
// get name of room from text file in storage area and send to GUI display
...
// get name of load from text file in storage area and send to GUI display
